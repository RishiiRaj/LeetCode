class Solution {
    public int largestOverlap(int[][] A, int[][] B) {
        ...
        Map<String, Integer> map = new HashMap<>(); // map to map the vector 
        //(from a pixel in A to a pixel in B) to its count
        for (int[] pa : la)
            for (int[] pb : lb) {
                String s = (pa[0] - pb[0]) + " " + (pa[1]-pb[1]); // get the vector from a pixel in A 
                //to a pixel in B
